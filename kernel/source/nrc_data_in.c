    ...
    struct nrc_din_stream_api   stream_api)
{
    ...
    else if (strcmp(cfg_msg_type, "buf") == 0) {
    ...
    bool_t                      *more_to_read)
{
    ...
    *more_to_read = FALSE;
    ...
    if (self->msg_type == NRC_MSG_TYPE_DATA_AVAILABLE) {
        ...
        if (bytes_to_read > 0) {
            if (bytes_to_read > pars.max_size) {
                bytes_to_read = pars.max_size;
            }
