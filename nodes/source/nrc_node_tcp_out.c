...
#define NRC_N_TCP_OUT_TYPE            (0x014323D2)
...
struct nrc_node_tcp_out {
    ...
    enum nrc_node_tcp_out_state     state;          // Node state
    nrc_tcp_t                       tcp;            // NRC tcp port
    struct nrc_tcp_writer           writer;         // Writer notification data
...
static nrc_node_t nrc_node_tcp_out_create(struct nrc_node_factory_pars *pars);

// Node api functions
static s32_t nrc_node_tcp_out_init(nrc_node_t self);
static s32_t nrc_node_tcp_out_deinit(nrc_node_t self);
static s32_t nrc_node_tcp_out_start(nrc_node_t self);
static s32_t nrc_node_tcp_out_stop(nrc_node_t self);
static s32_t nrc_node_tcp_out_recv_msg(nrc_node_t self, nrc_msg_t msg);
static s32_t nrc_node_tcp_out_recv_evt(nrc_node_t self, u32_t event_mask);

// Internal functions
static s32_t write_msg_buf(struct nrc_node_tcp_out *self, struct nrc_msg_buf *msg);
static s32_t write_msg_data_avail(struct nrc_node_tcp_out *self, struct nrc_msg_data_available *msg);

// Internal variables
const static s8_t*          _tag = "tcp-out";    // Used in NRC_LOG function
static struct nrc_node_api  _api;                // Node API functions

// Register node to node factory; called at system start
void nrc_node_tcp_out_register(void)
{
    s32_t result = nrc_factory_register_node_type("nrc-tcp-out", nrc_node_tcp_out_create);
    if (!OK(result)) {
...
nrc_node_t nrc_node_tcp_out_create(struct nrc_node_factory_pars *pars)
{
    struct nrc_node_tcp_out *self = NULL;

    if ((pars != NULL) && (strcmp("nrc-tcp-out", pars->cfg_type) == 0)) {
        self = (struct nrc_node_tcp_out*)nrc_os_node_alloc(sizeof(struct nrc_node_tcp_out));

        if (self != NULL) {
            // Future additions to node API will be set to NULL until supported by node
            memset(&_api, 0, sizeof(struct nrc_node_api));
            _api.init = nrc_node_tcp_out_init;
            _api.deinit = nrc_node_tcp_out_deinit;
            _api.start = nrc_node_tcp_out_start;
            _api.stop = nrc_node_tcp_out_stop;
            _api.recv_msg = nrc_node_tcp_out_recv_msg;
            _api.recv_evt = nrc_node_tcp_out_recv_evt;

            pars->api = &_api; // Return api functions for the node

            memset(self, 0, sizeof(struct nrc_node_tcp_out));
            ...
            self->type = NRC_N_TCP_OUT_TYPE; // For node pointer validity check
...
static s32_t nrc_node_tcp_out_init(nrc_node_t slf)
{
    struct nrc_node_tcp_out   *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
 
    if ((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE)) {
        switch (self->state) {
...
static s32_t nrc_node_tcp_out_deinit(nrc_node_t slf)
{
    struct nrc_node_tcp_out  *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
...
static s32_t nrc_node_tcp_out_start(nrc_node_t slf)
{
    struct nrc_node_tcp_out  *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
    ...
    if ((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE)) {
        switch (self->state) {
        case NRC_N_SERIAL_OUT_S_INITIALISED:
            result = nrc_tcp_open_writer(self->hdr.cfg_id, self->writer, &self->tcp);
            ...
            if (OK(result)) {
                ...
                self->state = NRC_N_SERIAL_OUT_S_ERROR;
                NRC_LOGE(_tag, "start(%s): could not open tcp_out", self->hdr.cfg_id);
            }
            break;
...
static s32_t nrc_node_tcp_out_stop(nrc_node_t slf)
{
    struct nrc_node_tcp_out   *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
    ...
    if ((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE)) {
        ...
        case NRC_N_SERIAL_OUT_S_STARTED:
            ...
            result = nrc_tcp_close_writer(self->tcp);
            self->tcp = NULL;
...
static s32_t nrc_node_tcp_out_recv_msg(nrc_node_t slf, nrc_msg_t msg)
{
    struct nrc_node_tcp_out  *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
    struct nrc_msg_hdr          *msg_hdr = (struct nrc_msg_hdr*)msg;

    if ((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE) && (msg_hdr != NULL)) {
        switch (self->state) {
        case NRC_N_SERIAL_OUT_S_STARTED:
...
static s32_t nrc_node_tcp_out_recv_evt(nrc_node_t slf, u32_t event_mask)
{
    struct nrc_node_tcp_out  *self = (struct nrc_node_tcp_out*)slf;
    s32_t                       result = NRC_R_INVALID_IN_PARAM;
    ...
    if ((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE)) {
        ...
        case NRC_N_SERIAL_OUT_S_STARTED_TX_BUF:
            ...
            if ((event_mask & NRC_N_SERIAL_OUT_EVT_ERROR) != 0) {
                //TODO
                //NRC_LOGW(_tag, "recv_evt(%s): tcp error %d", self->hdr.cfg_id, nrc_tcp_get_write_error(self->tcp));
            }
            break;
...
static s32_t write_msg_buf(struct nrc_node_tcp_out *self, struct nrc_msg_buf *msg)
{
    ...
    NRC_ASSERT((self != NULL) && (self->type == NRC_N_TCP_OUT_TYPE));
    NRC_ASSERT((msg != NULL) && (msg->hdr.type == NRC_MSG_TYPE_BUF));
    
    NRC_ASSERT(self->msg_buf == NULL);
    self->msg_buf = msg;

    result = nrc_tcp_write(self->tcp, msg->buf, msg->buf_size);
...
static s32_t write_msg_data_avail(struct nrc_node_tcp_out *self, struct nrc_msg_data_available *msg)
{
    ...
    self->state = NRC_N_SERIAL_OUT_S_STARTED;
    if (bytes > 0) {
        result = nrc_tcp_write(self->tcp, self->data_avail_buf, bytes);
