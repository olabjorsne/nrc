...
#define NRC_N_TCP_IN_TYPE               (0x18FF6565)

// Event bitmask for nrc_tcp callbacks
...
static nrc_node_t nrc_node_tcp_in_create(struct nrc_node_factory_pars *pars);

// Node api functions
static s32_t nrc_node_tcp_in_init(nrc_node_t self);
static s32_t nrc_node_tcp_in_deinit(nrc_node_t self);
static s32_t nrc_node_tcp_in_start(nrc_node_t self);
static s32_t nrc_node_tcp_in_stop(nrc_node_t self);
static s32_t nrc_node_tcp_in_recv_msg(nrc_node_t self, nrc_msg_t msg);
static s32_t nrc_node_tcp_in_recv_evt(nrc_node_t self, u32_t event_mask);
...
// Internal variables
const static s8_t*          _tag = "tcp-in";    // Used in NRC_LOG function
static struct nrc_node_api  _api;               // Node API functions

// Register node to node factory; called at system start
void nrc_node_tcp_in_register(void)
{
    s32_t result = nrc_factory_register_node_type("nrc-tcp-in", nrc_node_tcp_in_create);
    if (!OK(result)) {
...
nrc_node_t nrc_node_tcp_in_create(struct nrc_node_factory_pars *pars)
{
    struct nrc_node_tcp_in *self = NULL;

    if ((pars != NULL) && (strcmp("nrc-tcp-in", pars->cfg_type) == 0)) {
        self = (struct nrc_node_tcp_in*)nrc_os_node_alloc(sizeof(struct nrc_node_tcp_in));

        if (self != NULL) {
            // Future additions to node API will be set to NULL until supported by node
            memset(&_api, 0, sizeof(struct nrc_node_api));
            _api.init = nrc_node_tcp_in_init;
            _api.deinit = nrc_node_tcp_in_deinit;
            _api.start = nrc_node_tcp_in_start;
            _api.stop = nrc_node_tcp_in_stop;
            _api.recv_msg = nrc_node_tcp_in_recv_msg;
            _api.recv_evt = nrc_node_tcp_in_recv_evt;

            pars->api = &_api; // Return api functions for the node

            memset(self, 0, sizeof(struct nrc_node_tcp_in));
...
static s32_t nrc_node_tcp_in_deinit(nrc_node_t slf)
{
    struct nrc_node_tcp_in *self = (struct nrc_node_tcp_in*)slf;
    s32_t                  result = NRC_R_INVALID_IN_PARAM;
...
static s32_t nrc_node_tcp_in_start(nrc_node_t slf)
{
    struct nrc_node_tcp_in *self = (struct nrc_node_tcp_in*)slf;
    s32_t                  result = NRC_R_INVALID_IN_PARAM;
...
static s32_t nrc_node_tcp_in_stop(nrc_node_t slf)
{
    struct nrc_node_tcp_in *self = (struct nrc_node_tcp_in*)slf;
    s32_t                  result = NRC_R_INVALID_IN_PARAM;
...
static s32_t nrc_node_tcp_in_recv_msg(nrc_node_t slf, nrc_msg_t msg)
{
    struct nrc_node_tcp_in *self = (struct nrc_node_tcp_in*)slf;
    s32_t                  result = NRC_R_INVALID_IN_PARAM;
...
static s32_t nrc_node_tcp_in_recv_evt(nrc_node_t slf, u32_t event_mask)
{
    struct nrc_node_tcp_in *self = (struct nrc_node_tcp_in*)slf;
    s32_t                  result = NRC_R_INVALID_IN_PARAM;
